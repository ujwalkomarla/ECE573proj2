...
void *ClientReceiverThreadFunc(void *msg){
	...
	while(1){
		if(recvfrom(rcvdArgument->serverInfo->sockID,buf,sizeof(buf),0,(struct sockaddr *)&clientRcvrUDP,&clientRcvrUDPlen)<0) DieWithError("Client Receiver can't receive packets");
